...
**Two new bugs found, not yet fixed:**
1. **Custom School Manager → AI Consultation Hub wiring is broken.** Selecting a custom school as the active AI specialist visually highlights the button but doesn't actually switch anything — `ChatEngine.activeAgent` never updates, so the backend still gets the previously-selected built-in specialist's ID, and the "you're now speaking with X" persona/greeting never changes. Root cause: `ChatEngine`'s agent-selector click listeners are attached to a one-time DOM snapshot taken *before* custom-school buttons exist, and the handler's `this.AGENTS[targetAgent]` guard would block custom IDs even if it did fire. The custom school's actual rules text is unaffected by this bug and does still reach the AI (verified separately, via `getCombinedCustomRules()` → `systemPrompt` → `server.ts`), so answers are still influenced by the custom school — only the visible "who am I talking to" state is broken.
2. **Dashboard overflows horizontally on mobile (375px viewport).** `document.body.scrollWidth` is ~540px — about 165px of unavoidable horizontal scroll on the very first screen a mobile user sees. Caused by a few Dashboard hero-section grids/rows (`.hero-action-advice-grid`, `.school-quick-card`'s grid, `.archetype-pills-row`) missing from `styles.css`'s mobile breakpoint, unlike Foundation's BaZi/Zi Wei grids, which are handled correctly there. (Note for future audits: `resize_window` needs a page reload afterward before computed styles reflect the new viewport correctly — an early reading of this same issue was a stale-style false alarm on the *sidebar*, which does work correctly; the real bug is the Dashboard grids, confirmed after reloading.)
