...
**Decision:** Android is the first-choice platform (ahead of general web deployment), not published via AI Studio (privacy preference — personal birth/dream data). Architecture: a **routable AI backend** — on-device **Gemma 4 E4B** (LiteRT-LM; Apache 2.0, ungated on Hugging Face, downloaded on first launch — see ROADMAP.md for the license/model verification, including a correction of an earlier "Gemma 3n" assumption) on Android so sensitive readings never leave the device, cloud endpoints as fallback for web / when on-device isn't available. Full detail and live sub-phase tracking in [ROADMAP.md](ROADMAP.md) Phase 7.

**Status: the entire software build is done and verified.** Capacitor scaffolding, Android SDK setup, the native `AetheriaLLMPlugin.kt` (Kotlin, wraps LiteRT-LM), and the `api_service.js` routing layer (tries on-device first when available, falls back to the existing cloud chain otherwise, with a new Settings card for the one-time 3.66GB model download) are all complete. `./gradlew assembleDebug` succeeds end-to-end — fixed two real toolchain version mismatches along the way (Java 17 vs. required 21; then Gradle 8.14.3 vs. required 9.1.0 for JDK 25 host support; then Kotlin 2.1.0 vs. required 2.3.0 to match the LiteRT-LM library's metadata version — all documented in ROADMAP.md). What's *not* done: actually running the app on a device/emulator and confirming on-device inference works in practice — that needs hardware this environment doesn't have.
