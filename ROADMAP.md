...
4. **On-device LLM bridge** — 🔄 starting. Custom Capacitor plugin (Kotlin) wrapping LiteRT-LM to run **Gemma 4 E4B** on-device, exposing a JS-callable `generate()` method. Decided: E4B (over E2B) for quality, download-on-first-launch (over bundling — keeps the APK small; the 3.66GB model downloads once into app storage) for the model weights. LiteRT-LM Android API (verified 2026-08-19): `implementation("com.google.ai.edge.litertlm:litertlm-android:latest.release")`; Kotlin surface is `Engine(EngineConfig(modelPath = ...))` → `engine.initialize()` (can take up to ~10s, must run off the main thread) → `engine.createConversation()` → `conversation.sendMessage(prompt)` (sync) or `conversation.sendMessageAsync(prompt)` (Flow-based streaming). Relevant installed skills checked: `anthropic-skills:android-module-structure`, `anthropic-skills:android-build-logic-gradle` — their own guidance ("don't create a module/plugin for a single class," "don't over-engineer build logic for tiny gains in small repos") argues for adding this directly into the existing `:app` module rather than restructuring into their multi-module KMP template, which doesn't fit a single-module Capacitor WebView app.
5. **Routing layer** — not started. Extend `api_service.js`'s existing fallback chain (webhook → Gemini → offline text) to try the on-device plugin first when running inside the Android app.
6. **Testing/tuning** — not started. Compare on-device output quality against cloud, adjust prompts for the smaller model.

---

### Suggested immediate next step

Phases 1–4 are all done, and all three Phase 2 follow-ups are done too. Deployment target (Phase 0) is decided: **Android first**, via Capacitor + a routable AI backend (on-device Gemma 4 E4B, cloud fallback) — now **Phase 7**. Sub-phases 1-3 (Capacitor scaffolding, SDK setup, verified Gradle build) are done. Immediate next step: build the on-device LLM bridge (Phase 7.4). Phase 5 (general web deploy) and Phase 6 (polish) are paused behind Phase 7 per current direction.
